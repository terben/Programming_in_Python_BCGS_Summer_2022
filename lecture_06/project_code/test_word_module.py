...
#
# Test code for the word_module.py module
#
...
def test_get_word_score():
    ...
    for (word, n) in words.keys():
        score = wm.get_word_score(word, n)
...
def test_is_valid_word(word_list):
    ...
    # test 1
    word = "hello"
    handOrig = wm.get_frequency_dict(word)
    handCopy = handOrig.copy()

    if not wm.is_valid_word(word, handCopy, word_list):
    ...
    if not wm.is_valid_word(word, handCopy, word_list):
        ...
        if handCopy != handOrig:
            ...
            print("The word", word,
                  "should be in word_list - is it?", wordInWL)
    ...
    if  wm.is_valid_word(word, hand, word_list):
        print("FAILURE: test_is_valid_word()")
    ...
    if  not wm.is_valid_word(word, hand, word_list):
        print("FAILURE: test_is_valid_word()")
        print("\tExpected True, but got False for word: '"
              + word +"' and hand:", hand)
